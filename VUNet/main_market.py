...
import nn
import models
from batches_market import get_batches, plot_batch, postprocess, N_BPARTS
import deeploss
import batches_market_mine as bm
...
class Model(object):
    def __init__(self, opt, out_dir, logger):
        ...
        self.m_shape = 2*[opt.spatial_size] + [1]
        redux = 1
        self.redux = redux
        self.imgn_shape = 2*[opt.spatial_size//(2**redux)] + [N_BPARTS*3]
    ...
    def define_models(self):
        redux = self.redux
        n_latent_scales = 2
        n_scales = 1 + int(np.round(np.log2(self.img_shape[0]))) - redux
        n_filters = 32
    ...
    def likelihood_loss(self, x, params, mask):
    ...
    def define_graph(self):
        ...
        self.cn_init = tf.placeholder(
                tf.float32,
                shape = [self.init_batches * self.batch_size] + self.imgn_shape)
        self.m_init = tf.placeholder(
                tf.float32,
                shape=[self.init_batches * self.batch_size] + self.m_shape)
        ...
        self.m = tf.placeholder(
                tf.float32,
                shape = [self.batch_size] + self.m_shape)
        ...
        # maximize likelihood
        likelihood_loss = self.likelihood_loss(self.x, params, self.m)
        ...
        self.img_ops["x_masked"] = self.x * self.m
    ...
    def fit(self, batches, valid_batches = None):
        ...
        for batch in trange(start_step, self.lr_decay_end):
            X_batch, C_batch, XN_batch, CN_batch, M_batch = next(batches)
            feed_dict = {
                    self.xn: XN_batch,
                    self.cn: CN_batch,
                    self.x: X_batch,
    ...
    def log_result(self, result, **kwargs):
        ...
        if "img" in result:
            ...
            if self.valid_batches is not None:
                # validation run
                X_batch, C_batch, XN_batch, CN_batch, M_batch = next(self.valid_batches)
                feed_dict = {
                        self.xn: XN_batch,
                        self.cn: CN_batch,
                        self.x: X_batch,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_data_index", required = True, help = "path to training or testing data index")
    ...
    if opt.mode == "train":
        ...
        valid_batches = get_batches(data_shape, opt.test_data_index, mask = opt.mask, train = False)
    ...
    #     if not opt.checkpoint:
    #         opt.checkpoint = "log/2017-10-30T13:21:18/checkpoints/model.ckpt-100000"
    #     batch_size = opt.batch_size
    #     img_shape = 2*[opt.spatial_size] + [3]
    #     data_shape = [batch_size] + img_shape
    #     valid_batches = get_batches(data_shape, opt.test_data_index,
    #             mask = opt.mask, train = False)
    #     model = Model(opt, out_dir, logger)
    #     model.restore_graph(opt.checkpoint)
    #
    #     for step in trange(1):
    #         X_batch, C_batch, XN_batch, CN_batch, M_batch = next(valid_batches)
    #         bs = X_batch.shape[0]
    #         imgs = list()
    #         imgs.append(np.zeros_like(X_batch[0,...]))
    #         for r in range(bs):
    ...
    #             "transfer.png"))
        ...
        valid_batches = bm.get_batches(data_shape, opt.data_index,
                mas =opt.mask, train=False, return_keys=["source_imgs", "source_joints", "source_norm_imgs", "source_norm_joints", "target_imgs", "target_joints", "target_norm_imgs", "target_norm_joints", "from", "to", "target_imgs"], pairs_path=opt.pairs_path)
        model = Model(opt, out_dir, logger)
        model.restore_graph(opt.checkpoint)
        ...
        for iter in trange(valid_batches.n // batch_size):
            ...
            SX_batch, SC_batch, SXN_batch, SCN_batch, TX_batch, TC_batch, TXN_batch, TCN_batch, from_image, to_image, target_imgs = next(
                valid_batches)
            ...
            bm.plot_batch_mine_resize(condition_imgs, target_imgs, final_results, output_img_dir, from_image, to_image)
