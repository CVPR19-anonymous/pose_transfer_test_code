...
def plot_batch_mine_resize(src_imgs, target_poses, results, out_dir, from_image, to_image, resize_size=(128, 64)):
    """Save batch of images tiled."""
    # n_channels = src_imgs.shape[3]
    # if n_channels > 3:
    #     src_imgs = src_imgs[:, :, :, np.random.choice(n_channels, size=3)]
    ...
    target_poses = postprocess(target_poses)
    results = postprocess(results)
    single_height, single_width, n_channels = resize_size[0], resize_size[1], src_imgs.shape[3]
    for i in range(src_imgs.shape[0]):
        full_img = np.zeros((single_height, 3 * single_width, n_channels), dtype=src_imgs.dtype)
        full_img[:, 0:single_width, :] = cv2.resize(src_imgs[i, ...], (single_width, single_height), interpolation=cv2.INTER_CUBIC)
        full_img[:, single_width:2 * single_width, :] = cv2.resize(target_poses[i, ...], (single_width, single_height), interpolation=cv2.INTER_CUBIC)
        full_img[:, 2 * single_width:3 * single_width, :] = cv2.resize(results[i, ...], (single_width, single_height), interpolation=cv2.INTER_CUBIC)
        PIL.Image.fromarray(full_img).save(os.path.join(out_dir, "{}___{}_vis.jpg".format(from_image[i], to_image[i])))
...
class IndexFlow(object):
    ...
    def __next__(self):
        ...
        for i in batch_indices:
            ...
            batch["source_imgs"].append(I)

            target_path = os.path.join(self.basepath, "{}/{}".format(traintest, self.index["imgs"][target_name]))
            t_I = load_img(target_path, target_size=self.img_shape)
            t_I = cv2.warpAffine(t_I, self.stretchM, t_I.shape[:2])
            batch["target_imgs"].append(t_I)

            source_joints = self.index["joints"][source_name] * self.wh
            source_joints = cv2.transform(source_joints[None, ...], self.stretchM)
            batch["source_joints_coordinates"].append(source_joints[0])
            batch["source_joints"].append(make_joint_img(self.img_shape, self.jo, source_joints[0]))

            target_joints = self.index["joints"][target_name] * self.wh
            target_joints = cv2.transform(target_joints[None, ...], self.stretchM)
            batch["target_joints_coordinates"].append(target_joints[0])
            batch["target_joints"].append(make_joint_img(self.img_shape, self.jo, target_joints[0]))
